...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run 5-fold training/evaluation for VGG/DenseNet/ResNet/MobileNet/"
            "EfficientNet using ImageNet-pretrained weights on naively "
            "replicated (3x channel copy) grayscale thyroid scintigraphy images, "
            "trained on the GAN-augmented 5-fold split (real-only validation, all "
            "synthetic rows in every training fold). Stacks the two independently "
            "validated ingredients: pretrained naive-replication + GAN augmentation."
        )
    )
    parser.add_argument(
        "--fold-root",
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("classification_pretrained_replication_5fold_gan_augmented_outputs"),
...
def load_fold_split_df(
    ...
    df["label_idx"] = df["label_group"].map(label_to_idx).astype(int)
    return df
...
def compute_fold_metrics(
    ...
    report = classification_report(
        y_true,
        y_pred,
        target_names=[f"group_{idx_to_label[i]}" for i in range(num_classes)],
        output_dict=True,
        zero_division=0,
    )
    ...
    for model_name in model_names:
        ...
        for fold_id in fold_ids:
            ...
            sns.heatmap(
                ...
                fmt="d",
                cmap="Blues",
                xticklabels=[str(idx_to_label[i]) for i in range(num_classes)],
                yticklabels=[str(idx_to_label[i]) for i in range(num_classes)],
                ax=axes[0],
            )
            ...
            for i in range(num_classes):
                plt.plot(
                    ...
                    linewidth=1.6,
                    label=f"Group {idx_to_label[i]} (AUC={metrics_pack['auc_by_class'][i]:.3f})",
                )
            plt.plot(
...
def save_gradcam_plots(
    ...
    for _, row in best_models_df.iterrows():
        ...
        for r, sample_i in enumerate(sample_ids):
            ...
            plt.title(f"CAM\nPred group={idx_to_label[cam_class]}")
            plt.axis("off")
            plt.subplot(len(sample_ids), 3, 3 * r + 3)
            plt.imshow(overlay)
            plt.title(f"Overlay\nTrue={idx_to_label[true_idx]} | Pred={idx_to_label[pred_idx]}")
            plt.axis("off")
        ...
        fold_summary_rows.append(
            {
                ...
                "train_class_dist": dict(train_df_tmp["label_group"].value_counts().sort_index()),
                "val_class_dist": dict(val_df_tmp["label_group"].value_counts().sort_index()),
            }
        )
    ...
    for model_name in args.models:
        ...
        for fold_id in fold_ids:
            ...
            for c in range(num_classes):
                pred_df[f"prob_group_{idx_to_label[c]}"] = probs[:, c]
            pred_df.to_csv(
